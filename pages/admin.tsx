...
export default function Admin() {
  ...
  const [actionToConfirm, setActionToConfirm] = useState({ action: null, userId: null, userName: null });
  ...
  const loadData = async () => {
    ...
    try {
      ...
      // Update expired users to blocked status
      ...
      setUsers(updatedUsers.filter(u => u.email !== me.email));
      setCurrentUser(me);
    } catch (error) {
  ...
  const calculatePlanExpiration = (approvedAt, planType) => {
  ...
  const handleUpdateUserStatus = async (userId, newStatus, planType = null) => {
    try {
      const updateData = {
        status: newStatus,
      ...
      if (newStatus === 'approved' && planType) {
        ...
        // Clear plan details if user is denied or blocked
  ...
  const confirmAction = (action, userId, userName) => {
  ...
  const executeConfirmedAction = async () => {
    ...
    if (action === 'delete') {
      ...
      // This path is for basic approval without plan selection,
      // the primary plan-based approval uses handleUpdateUserStatus directly from UserActions.
